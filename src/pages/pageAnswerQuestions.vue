  ...
  <div>
    <div v-if="formularioActual === 1">
      ...
      <div v-for="(item, indexHeader) in getIdentify" :key="indexHeader" class="mb-4">
        ...
        <div>
          <label v-for="(option, index) in item.optionsList" :key="index"
            class="flex items-center space-x-2 cursor-pointer">
            <input :name="'option-' + indexHeader" type="radio" :value="option.id"
              v-model="getIdentify[indexHeader].optionValue" class="form-radio text-blue-500" />
            <span :for="`option-${index}`" class="text-gray-700">{{ option.nameOption }}</span>
          </label>
        </div>
      </div>
      <div class="flex float-left mt-5"> 
        <button @click=" router.push({ name: 'respuest' })"
      ...
      <div class="flex justify-end mt-5">
        <button @click="changePage(2, 'identify')"
    ...
    <div v-if="formularioActual === 3">
      ...
      <div v-for="(item, indexHeader) in getDetected" :key="indexHeader" class="mb-4">
        ...
        <div>
          <label v-for="(option, index) in item.optionsList" :key="index"
            class="flex items-center space-x-2 cursor-pointer">
            <input :name="'option-' + indexHeader" type="radio" :value="option.id"
              v-model="getDetected[indexHeader].optionValue" class="form-radio text-blue-500" />
      ...
      <div class="flex justify-end mt-5">
        <button @click="changePage(4, 'detected')"
          class="bg-blue-500 ml-2 text-white py-2 px-4 rounded-md shadow-sm  focus:outline-none">
          Siguiente
        </button>
      </div>
    ...
    <div v-if="formularioActual === 2">
      ...
      <div v-for="(item, indexHeader)  in getProtect" :key="indexHeader" class="mb-4">
        ...
        <div>
          <label v-for="option in item.optionsList" :key="option.value"
            class="flex items-center space-x-2 cursor-pointer">
            <input :name="'option-' + indexHeader" type="radio" :value="option.id"
              v-model="getProtect[indexHeader].optionValue" class="form-radio text-blue-500" />
            <span class="text-gray-700">{{ option.nameOption }}</span>
          </label>
        </div>
      </div>
      ...
      <div class="flex justify-end mt-5">
        <button @click="changePage(3, 'protect')"
          class="bg-blue-500 ml-2 text-white py-2 px-4 rounded-md shadow-sm  focus:outline-none">
          Siguiente
        </button>
      </div>
      ...
      <div v-for="(item, indexHeader) in getRecover" :key="indexHeader" class="mb-4">
        ...
        <div>
          <label v-for="option in item.optionsList" :key="option.value"
            class="flex items-center space-x-2 cursor-pointer">
            <input :name="'option-' + indexHeader" type="radio" :value="option.id"
              v-model="getRecover[indexHeader].optionValue" class="form-radio text-blue-500" />
            <span class="text-gray-700">{{ option.nameOption }}</span>
          </label>
        </div>
      </div>
    ...
    <div v-if="formularioActual === 4">
      ...
      <div v-for="(item, indexHeader) in getRespond" :key="indexHeader" class="mb-4">
        ...
        <div>
          <label v-for="option in item.optionsList" :key="option.value"
            class="flex items-center space-x-2 cursor-pointer">
            <input :name="'option-' + indexHeader" type="radio" :value="option.id"
              v-model="getRespond[indexHeader].optionValue" class="form-radio text-blue-500" />
            <span class="text-gray-700">{{ option.nameOption }}</span>
          </label>
        </div>
      </div>
      ...
      <div class="flex justify-end mt-5">
        <button @click="changePage(5, 'respond')"
          class="bg-blue-500 ml-2 text-white py-2 px-4 rounded-md shadow-sm  focus:outline-none">
          Siguiente
        </button>
      </div>

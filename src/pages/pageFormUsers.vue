...
const buttonLabel = ref('Iniciar Test'); // Inicializar con "Iniciar Test"
const isLoading = ref(true); // Bandera para determinar si se está cargando


// Función para actualizar el texto del botón según el estado del test
const updateButtonLabel = async () => {
    const isFinished = getShowFinished.value;
    buttonLabel.value = isFinished ? 'Reiniciar Test' : 'Iniciar Test';
    isLoading.value = false; // Termina la carga después de obtener el valor
...
onMounted(async () => {
  await fetchQuestions();
  await updateButtonLabel(); // Actualizar el texto del botón al cargar la página
});
</script>
